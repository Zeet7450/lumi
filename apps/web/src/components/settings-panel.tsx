...
export function SettingsPanel({ audience }: { audience: "petugas" | "warga" }) {
  ...
  useEffect(() => {
    ...
    if (stored) setSettings({ ...defaults, ...JSON.parse(stored) as Settings });
      ...
      const theme = window.localStorage.getItem("lumi-theme");
      if (theme === "light" || theme === "dark") setSettings((current) => ({ ...current, theme }));
    }
    setHydrated(true);
  }, [audience]);
  useEffect(() => {
    ...
    window.localStorage.setItem("lumi-theme", settings.theme);
