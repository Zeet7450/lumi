...
const bands = [
  [50, "#138A53", "Baik 0–50"], [100, "#D4A72C", "Sedang 51–100"], [150, "#E67E22", "Tidak sehat bagi kelompok sensitif 101–150"],
  [200, "#D94841", "Tidak sehat 151–200"], [300, "#7B3F98", "Sangat tidak sehat 201–300"], [500, "#6D1F2C", "Berbahaya 301–500"]
] as const;
function aqiColor(aqi: number | null) { return aqi === null ? "#8793A4" : bands.find(([limit]) => aqi <= limit)?.[1] ?? "#6D1F2C"; }
function aqiLabel(aqi: number | null) { return aqi === null ? "Belum ada data" : bands.find(([limit]) => aqi <= limit)?.[2] ?? "Berbahaya 301–500"; }
...
export default function OpsMapClient({ points }: { points: Point[] }) {
  ...
  return <section className="command-panel live-map" aria-labelledby="risk-map-title">
    ...
    <div className="leaflet-frame"><button className="map-fullscreen" type="button" onClick={toggleFullscreen} aria-label="Buka peta layar penuh" title="Layar penuh"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3H4a1 1 0 0 0-1 1v4m13-5h4a1 1 0 0 1 1 1v4M3 16v4a1 1 0 0 0 1 1h4m13-5v4a1 1 0 0 1-1 1h-4" /></svg></button><MapContainer center={[-1.12, 114.2]} zoom={5} minZoom={4} maxZoom={16} scrollWheelZoom className="leaflet-canvas" aria-label="Peta interaktif kualitas udara Kalimantan"><TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><CaptureMap onReady={setMap} /><FocusSelection point={selected} />
      {visible.map((point) => { const coords = boundaryCoordinates(point); return coords.length >= 3 ? <Polygon key={`${point.id}-boundary`} positions={coords} pathOptions={{ color: aqiColor(point.aqi), fillColor: aqiColor(point.aqi), fillOpacity: .12, weight: selected?.id === point.id ? 2 : 1 }} eventHandlers={{ click: () => setSelected(point) }} /> : null; })}
      {showRadius && visible.filter((point) => point.pm25 !== null).map((point) => <Circle key={`${point.id}-radius`} center={[point.latitude, point.longitude]} radius={point.radiusKm * 1000} pathOptions={{ color: aqiColor(point.aqi), fillColor: aqiColor(point.aqi), fillOpacity: selected?.id === point.id ? .16 : .07, weight: selected?.id === point.id ? 2 : 1 }} eventHandlers={{ click: () => setSelected(point) }} />)}
      {visible.map((point) => <CircleMarker key={point.id} center={[point.latitude, point.longitude]} radius={selected?.id === point.id ? 10 : 7} pathOptions={{ color: "#FFFFFF", fillColor: aqiColor(point.aqi), fillOpacity: 1, weight: 2.5 }} eventHandlers={{ click: () => setSelected(point) }}><Tooltip direction="top" offset={[0, -8]}>{point.city} · {point.aqi === null ? "menunggu data" : `AQI ${point.aqi}`}</Tooltip></CircleMarker>)}</MapContainer></div>
    ...
    {selected ? <div className="map-detail" aria-live="polite"><span className="status-dot" style={{ background: aqiColor(selected.aqi) }} /><div className="map-detail-body"><div className="map-detail-heading"><div><strong>{selected.city}</strong><p>{selected.province} · {selected.district}</p></div><span className="badge">{aqiLabel(selected.aqi)}</span></div><dl className="risk-facts"><div><dt>AQI</dt><dd>{selected.aqi ?? "–"}</dd></div><div><dt>PM2.5</dt><dd>{selected.pm25 === null ? "–" : `${selected.pm25} µg/m³`}</dd></div><div><dt>Radius</dt><dd>{selected.pm25 === null ? "–" : `${selected.radiusKm} km`}</dd></div><div><dt>Pembaruan</dt><dd>{selected.observedAt ? new Intl.DateTimeFormat("id-ID", { timeStyle: "short", timeZone: "Asia/Jakarta" }).format(new Date(selected.observedAt)) + " WIB" : "Menunggu data"}</dd></div></dl>{selected.hazards.length ? <div className="hazard-list" aria-label="Indikasi kejadian">{selected.hazards.map((hazard) => <span key={`${hazard.type}-${hazard.severity}`}><HazardGlyph type={hazard.type} />{hazardLabel(hazard.type)} · tingkat {hazard.severity}</span>)}</div> : <small>Tidak ada indikasi kejadian dari telemetri terbaru.</small>}</div></div> : <p className="muted">Belum ada titik pemantauan.</p>}
    <div className="map-location-list" aria-label="Daftar wilayah peta">{visible.map((point) => <button key={`select-${point.id}`} className={selected?.id === point.id ? "is-selected" : ""} type="button" onClick={() => setSelected(point)}><span style={{ background: aqiColor(point.aqi) }} />{point.city}<strong>{point.aqi === null ? "–" : `AQI ${point.aqi}`}</strong></button>)}</div>
  </section>;
}
