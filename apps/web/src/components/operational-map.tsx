...
function FocusSelectedPoint({ point, request }: { point: OperationalPoint; request: number }) {
  const map = useMap();
  useEffect(() => {
    if (!request) return;
    map.flyTo(point.coordinates, Math.max(map.getZoom(), 11), { animate: true, duration: 0.45 });
  }, [map, point, request]);
  return null;
}
...
export function OperationalMap() {
  ...
  return <section className="operational-map" aria-labelledby="operational-map-title">
    ...
    <div className="operational-map-grid"><div ref={frame} className="operational-map-frame"><FullscreenButton target={frame} /><MapContainer center={[-0.35, 110.6]} zoom={7} minZoom={5} maxZoom={13} scrollWheelZoom className="operational-map-canvas" aria-label="Peta operasional data uji Kalimantan Barat"><TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><FitPoints points={points} /><FocusSelectedPoint point={selected} request={focusRequest} /><ResizeForSidebar />{points.map((point) => <CircleMarker key={point.id} center={point.coordinates} radius={point.id === selected.id ? 10 : 8} pathOptions={{ color: point.incidentStatus === "Tidak ada" ? "#ffffff" : "#7f1d1d", fillColor: aqiColor(point.aqi), fillOpacity: 1, weight: point.incidentStatus === "Tidak ada" ? 2 : 5 }} eventHandlers={{ click: () => selectPoint(point.id) }}><Tooltip>{point.name} · AQI {point.aqi}{point.incidentStatus !== "Tidak ada" ? ` · ${point.incidentStatus}` : ""}</Tooltip></CircleMarker>)}</MapContainer><span className="map-data-label">Visualisasi lokal · data sintetis</span><div className="aqi-legend" aria-label="Legenda AQI"><strong>AQI</strong><span><i className="aqi-good" />0–50</span><span><i className="aqi-moderate" />51–100</span><span><i className="aqi-sensitive" />101–150</span><span><i className="aqi-unhealthy" />151–200</span><span><i className="aqi-very-unhealthy" />201–300</span><span><i className="aqi-hazardous" />301+</span><em>Cincin gelap = insiden sintetis</em></div></div></div>
